...
configFile = installDir + "/darknexus.cfg"
print(installDir)
config = ConfigParser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('darknexus', 'toolDir')
logDir = installDir + config.get('darknexus', 'logDir')
yes = config.get('darknexus', 'yes').split()
color_random=[color.HEADER,color.IMPORTANT,color.NOTICE,color.OKBLUE,color.OKGREEN,color.WARNING,color.RED,color.END,color.UNDERLINE,color.LOGGING]
random.shuffle(color_random)
darknexuslogo = color_random[0] + '''                                                                      
________                   ___      ___      ___                                      
`MMMMMMMb.                 `MM      `MM\     `M'                                      
 MM    `Mb                  MM       MMM\     M                                       
 MM     MM    ___   ___  __ MM   __  M\MM\    M   ____  ____   ___ ___   ___   ____   
 MM     MM  6MMMMb  `MM 6MM MM   d'  M \MM\   M  6MMMMb `MM(   )P' `MM    MM  6MMMMb\ 
 MM     MM 8M'  `Mb  MM69 " MM  d'   M  \MM\  M 6M'  `Mb `MM` ,P    MM    MM MM'    ` 
 MM     MM     ,oMM  MM'    MM d'    M   \MM\ M MM    MM  `MM,P     MM    MM YM.      
 MM     MM ,6MM9'MM  MM     MMdM.    M    \MM\M MMMMMMMM   `MM.     MM    MM  YMMMMb  
 MM     MM MM'   MM  MM     MMPYM.   M     \MMM MM         d`MM.    MM    MM      `Mb 
 MM    .M9 MM.  ,MM  MM     MM  YM.  M      \MM YM    d9  d' `MM.   YM.   MM L    ,MM 
_MMMMMMM9' `YMMM9'Yb_MM_   _MM_  YM._M_      \M  YMMMM9 _d_  _)MM_   YMMM9MM_MYMMMM9  
                '''
darknexusPrompt = "darknexus ~# "
alreadyInstalled = "Already Installed"
continuePrompt = "\nClick [Return] to continue"

termsAndConditions = color.NOTICE + '''
I shall not use darknexus to:
...
mrmorfin = color.NOTICE + '''
Hello,

As we all know, Mr. Morfin 4.0 is comming out - the end of Mr. Morfin.
...
Thank you for all the sourport over the years, the DarkNexus team thanks you!
Feel free to join the NEW DISCORD!!!
Anything Mr. Morfin will be on the server!
...
'''
def agreement():
    while not config.getboolean("darknexus", "agreement"):
        clearScr()
        print(termsAndConditions)
        print(mrrobot4)
        agree = raw_input("You must agree to our terms and conditions first (Y/n) ").lower()
        if agree in yes:
            config.set('darknexus', 'agreement', 'true')

class darknexus:
    def __init__(self):
        clearScr()
        self.createFolders()
        print (darknexuslogo + color.RED + '''
       }--------------{+} Coded By Morfin {+}--------------{
       }--------{+}  GitHub.com/darknexus11/darknexus {+}--------{
    ''' + color.END + '''
       ...
       {99}-EXIT\n
     ''')
        choice = raw_input(darknexusPrompt)
        clearScr()
        if choice == "1":
    ...
    Yb      Yb   dP 88 Y88   88   88"Yb  88 88""Yb o.`Y8b
     ...
     ''')
        contributorsURL = 'https://api.github.com/repos/darknexus11/darknexus/contributors'
    ...
    def update(self):
        os.system("git clone --depth=1 https://github.com/darknexus11/darknexus.git")
        os.system("cd darknexus && bash ./update.sh")
        os.system("darknexus")
...
class sniffingSpoofingMenu:
    ...
    def __init__(self):
        ...
        choice6 = raw_input(darknexusPrompt)
        clearScr()
        if choice6 == "1":
            setoolkit()
        elif choice6 == "2":
...
class webHackingMenu:
    ...
    def __init__(self):
        ...
        print("   {99}-Back To Main Menu \n")
        choiceweb = raw_input(darknexusPrompt)
        clearScr()
        if choiceweb == "1":
            maine()
        elif choiceweb == "2":
...
class postExploitationMenu:
    ...
    def __init__(self):
        ...
        choice11 = raw_input(darknexusPrompt)
        clearScr()
        if choice11 == "1":
            sitechecker()
        elif choice11 == "2":
...
class informationGatheringMenu:
    ...
    def __init__(self):
        ...
        print("  {8}--Crips\n  ")
        print("  {99}-Back To Main Menu \n")
        choice2 = raw_input(darknexusPrompt)
        clearScr()
        if choice2 == "1":
            nmap()
        elif choice2 == "2":
...
class wirelessTestingMenu:
    ...
    def __init__(self):
        ...
        choice4 = raw_input(darknexusPrompt)
        clearScr()
        if choice4 == "1":
            reaver()
        elif choice4 == "2":
...
class exploitationToolsMenu:
    ...
    def __init__(self):
        ...
        choice5 = raw_input(darknexusPrompt)
        clearScr()
        if choice5 == "1":
            atscan()
        elif choice5 == "2":
